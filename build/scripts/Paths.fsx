    ...
    type ProfilerTooling(path) =
        ...
        member this.Bootstrap = fun _ ->
            if (not (Directory.Exists dotTraceDirectory)) then
                ...
                let zipFile = sprintf "%s.zip" dotTraceDirectory
                use webClient = new WebClient()
            ...
            if (not (Directory.Exists selfProfilingDirectory)) then
                trace (sprintf "No JetBrains Self Profiling tooling found in %s. Downloading now" selfProfilingDirectory) 
                let url = sprintf "http://download-cf.jetbrains.com/resharper/%s.zip" selfProfilingTools      
                let zipFile = sprintf "%s.zip" selfProfilingDirectory
                use webClient = new WebClient()
                webClient.DownloadFile(url, zipFile)
                System.IO.Compression.ZipFile.ExtractToDirectory(zipFile, selfProfilingDirectory)
                File.Delete zipFile
                trace "JetBrains Self Profiling tooling downloaded"
