...
namespace Nest500
{
	public partial interface IElasticClient
	{
		...
		/// <inheritdoc/>
		Task<ICountResponse> CountAsync<T>(Func<CountDescriptor<T>, ICountRequest> selector = null, CancellationToken cancellationToken = default(CancellationToken))
			where T : class;
	...
	public partial class ElasticClient
	{
		...
		/// <inheritdoc/>
		public Task<ICountResponse> CountAsync<T>(Func<CountDescriptor<T>, ICountRequest> selector = null, CancellationToken cancellationToken = default(CancellationToken))
			where T : class =>
			this.CountAsync<T>(selector.InvokeOrDefault(new CountDescriptor<T>()), cancellationToken);
