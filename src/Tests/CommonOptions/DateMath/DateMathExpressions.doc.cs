...
namespace Tests.CommonOptions.DateMath
{
	public class DateMathEpressions
	{
		...
		[U] public void SimpleExpressions()
		{
			...
			*/
			Expect("now").WhenSerializing(Nest252.DateMath.Now);
			Expect("2015-05-05T00:00:00").WhenSerializing(Nest252.DateMath.Anchored(new DateTime(2015,05, 05)));
			...
			Expect(nonsense).WhenSerializing<Nest252.DateMath>(nonsense)
			...
			Expect("2015-05-05T00:00:00").WhenSerializing<Nest252.DateMath>(date)
		...
		[U] public void ComplexExpressions()
		{
			...
			*/
			Expect("now+1d").WhenSerializing(
				Nest252.DateMath.Now.Add("1d"));

			/** Including multiple operations */
			Expect("now+1d-1m").WhenSerializing(
				Nest252.DateMath.Now.Add("1d").Subtract(TimeSpan.FromMinutes(1)));
			...
			*/
			Expect("2015-05-05T00:00:00||+1d-1m").WhenSerializing(
				Nest252.DateMath.Anchored(new DateTime(2015,05,05))
					.Add("1d")
					.Subtract(TimeSpan.FromMinutes(1)));
		}
		...
		[U] public void FractionalsUnitsAreDroppeToIntegerPart()
		{
			...
			*/
			Expect("now+25h").WhenSerializing(
				Nest252.DateMath.Now.Add(TimeSpan.FromHours(25)));
			...
			Expect("now+90001s").WhenSerializing(
				Nest252.DateMath.Now.Add(TimeSpan.FromHours(25).Add(TimeSpan.FromSeconds(1))));

			Expect("now+90000001ms").WhenSerializing(
				Nest252.DateMath.Now.Add(TimeSpan.FromHours(25).Add(TimeSpan.FromMilliseconds(1))));

			Expect("now+1y").WhenSerializing(
				Nest252.DateMath.Now.Add("1y"));

			Expect("now+52w").WhenSerializing(
				Nest252.DateMath.Now.Add(TimeSpan.FromDays(7 * 52)));
