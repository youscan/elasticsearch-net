...
namespace Tests.CommonOptions.DistanceUnit
{
	public class DistanceUnits
	{
		...
		public void Constructor()
		{
			...
			var unitComposedWithUnits = new Distance(25, Nest252.DistanceUnit.Meters);
		...
		*/
		...
		public void UsingDifferentUnits()
		{
			...
			*`mm` (Millimeters)
			*/
			Expect("2.0mm").WhenSerializing(new Distance(2, Nest252.DistanceUnit.Millimeters));

			/**
			*`cm` (Centimeters)
			*/
			Expect("123.456cm").WhenSerializing(new Distance(123.456, Nest252.DistanceUnit.Centimeters));

			/**
			*`m` (Meters)
			*/
			Expect("400.0m").WhenSerializing(new Distance(400, Nest252.DistanceUnit.Meters));

			/**
			*`km` (Kilometers)
			*/
			Expect("0.1km").WhenSerializing(new Distance(0.1, Nest252.DistanceUnit.Kilometers));

			/** ===== Imperial
			*`in` (Inches)
			*/
			Expect("43.23in").WhenSerializing(new Distance(43.23, Nest252.DistanceUnit.Inch));

			/**
			*`ft` (Feet)
			*/
			Expect("3.33ft").WhenSerializing(new Distance(3.33, Nest252.DistanceUnit.Feet));

			/**
			*`yd` (Yards)
			*/
			Expect("9.0yd").WhenSerializing(new Distance(9, Nest252.DistanceUnit.Yards));

			/**
			*`mi` (Miles)
			*/
			Expect("0.62mi").WhenSerializing(new Distance(0.62, Nest252.DistanceUnit.Miles));

			/**
			*`nmi` or `NM` (Nautical Miles)
			*/
			Expect("45.5nmi").WhenSerializing(new Distance(45.5, Nest252.DistanceUnit.NauticalMiles));
		}
	}
}
